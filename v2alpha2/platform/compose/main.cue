...
import (
	...
	v2alpha2schema "jacero.io/oam/v2alpha2/schema/generic"
	v2alpha2compose "jacero.io/oam/v2alpha2/schema/compose"
)

#ContainerSpecToService: {
	I=input!: v2alpha2schema.#ContainerSpec
	#name!:   string
	result: v2alpha2compose.#Service & {
		hostname:       #name
		container_name: #name
...
#ToVolumes: {
	P=prefix!: string
	I=input!: [...v2alpha2schema.#Volume]
	result: {[string]: v2alpha2compose.#Volume} & {
		for volume in I {
			if volume.type == "emptyDir" {
				"\(volume.name)": {
					name:   "\(P)-\(volume.name)"
...
#ToServicePullPolicy: {
	I=input!: v2alpha2schema.#PullPolicy
	result: v2alpha2compose.#PullPolicy & {
		if I == "IfNotPresent" {
			"if_not_present"
		}
		if I == "Always" {
...
#ToServiceRestartPolicy: {
	I=input!: v2alpha2schema.#RestartPolicy
	result: v2alpha2compose.#RestartPolicy & {
		if I == "Always" {
			"always"
		}
		if I == "OnFailure" {
...
#ToServiceEnv: {
	I=input!: [...v2alpha2schema.#EnvVar]
	result: v2alpha2compose.#list_or_dict & {
		for value in I {
...
#ToServiceDeployResources: {
	I=input!: v2alpha2schema.#ResourceRequirements
	// v2alpha2compose.#deployment.resources & 
	result: {
		if I.requests != _|_ {
			reservations: {
